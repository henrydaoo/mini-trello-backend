...
public class TaskListController {
    ...
    public ResponseEntity<TaskListResponse> createList(Authentication authentication,
                                                       @PathVariable Long boardId,
                                                       @Valid @RequestBody TaskListRequest request) {
        TaskListResponse response = taskListService.createList(authentication.getName(), boardId, request);
        return ResponseEntity.status(HttpStatus.CREATED).body(response);
    }

    @PutMapping("/api/lists/{listId}")
    public ResponseEntity<TaskListResponse> updateList(Authentication authentication,
    ...
    public ResponseEntity<TaskListResponse> updatePosition(Authentication authentication,
                                                           @PathVariable Long listId,
                                                           @Valid @RequestBody TaskListPositionRequest request) {
        return ResponseEntity.ok(taskListService.updatePosition(authentication.getName(), listId, request));
    }
