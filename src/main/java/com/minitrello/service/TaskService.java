...
public class TaskService {
    ...
    public List<TaskResponse> getTasksForList(String username, Long listId, Long assigneeIdFilter) {
        ...
        List<Task> tasks = assigneeIdFilter != null
    ...
    private User resolveAssignee(Board board, Long assigneeId) {
        ...
        boolean canBeAssigned = boardAccessService.isOwner(board, assignee) || boardAccessService.isMember(board, assignee);
        if (!canBeAssigned) {
            throw new ForbiddenOperationException("Only the board owner or a board member can be assigned to a task");
        }
        return assignee;
    }
