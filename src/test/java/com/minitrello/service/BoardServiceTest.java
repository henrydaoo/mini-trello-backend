...
public class BoardServiceTest {
    ...
    @BeforeEach
    void setUp() {
        owner = User.builder().id(1L).username("owner").email("owner@test.com").passwordHash("h").build();
        newMember = User.builder().id(2L).username("jane").email("jane@example.com").passwordHash("h").build();
    ...
    void updateBoard_nonOwner_throwsForbidden_andDoesNotSave() {
        ...
        doThrow(new ForbiddenOperationException("Only the board owner can perform this action"))
                .when(boardAccessService).assertIsOwner(board, newMember);
    ...
    void addMember_validNewMember_savesAndReturnsUpdatedBoard() {
        ...
        BoardResponse response = boardService.addMember("owner", 10L, new AddMemberRequest("jane@example.com"));
    ...
    void removeMember_notAMember_throwsBoardMemberNotFoundException() {
        ...
        assertThrows(BoardMemberNotFoundException.class, () -> boardService.removeMember("owner", 10L, 99L));
